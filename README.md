...
- Added **Merge all GPX files into one output per layer type**.
- Added a configurable prefix for merged output filenames.
- Merged outputs can include source-layer name and source-path fields when
  supported by the installed QGIS version.
- Improved the completion summary by separating:
  - converted or merged outputs;
  - source layers included;
  - missing or empty GPX layers;
  - existing outputs skipped;
  - failed outputs.
- Missing GPX sublayers are no longer presented as conversion errors.

## Version 1.2.0

### Results panel

- Added a structured results table with one row per GPX file and layer.
- Displays source file, GPX layer, status, feature count, output path and
  the full GDAL error or warning message.
- Added a conversion summary separating converted, merged, included,
  missing/empty, existing and failed operations.
- Results can be exported to a UTF-8 CSV report.

### Multiple output formats

The plugin can now convert GPX data to:

- ESRI Shapefile;
- GeoPackage;
- GeoJSON;
- KML;
- CSV with geometry stored as WKT.

When merge mode and GeoPackage are selected, all merged GPX layer types are
stored inside a single `.gpkg` file.

### Background processing and cancellation

- Conversion now runs through the QGIS Task Manager.
- The QGIS interface remains responsive during large conversions.
- Added a **Cancel** button.
- The active GDAL process is terminated when cancellation is requested.
- Outputs completed before cancellation are preserved.
- QGIS layers and interface widgets are only accessed after the task returns
  to the main thread.

### Source provenance in merged outputs

Merged outputs include the following fields:

- `source_file`;
- `source_path`;
- `source_layer`.

These fields make it possible to trace each merged feature back to its
original GPX file.
...
`https://github.com/Jubilio/gpx-batch-converter`

Issue reports should be submitted through:

`https://github.com/Jubilio/gpx-batch-converter/issues`

The plugin has no external Python package dependency. It uses `ogr2ogr` and
`ogrinfo` from the GDAL environment supplied by, or made available to, QGIS.
